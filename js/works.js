/* works.js — data/works.json を読み込みエディトリアル・ギャラリー描画＋カテゴリフィルタ
   使い方: <div data-works data-works-filters="true"></div>
   - data-works-limit  : 表示件数上限（省略で全件）
   - data-works-filters: "false" でフィルタ非表示
   items[].size : "lg" | "wide" | "tall"（省略は通常）で誌面のサイズ差を出す
*/
(function () {
  "use strict";
  ...
  var sizeClass = function (s) {
    return { lg: " work--lg", wide: " work--wide", tall: " work--tall" }[s] || "";
  };

  fetch("data/works.json", { cache: "no-cache" })
    .then(function (r) { if (!r.ok) throw new Error("works.json " + r.status); return r.json(); })
    .then(render)
    .catch(function (err) {
  ...
  function render(data) {
    ...
    if (showFilters && cats.length) {
      ...
      filtersEl.appendChild(makeFilter("all", "すべて", true));
      cats.forEach(function (c) { filtersEl.appendChild(makeFilter(c.id, c.label, false)); });
      filtersEl.addEventListener("click", function (e) {
        var btn = e.target.closest(".filter");
        if (!btn) return;
        filtersEl.querySelectorAll(".filter").forEach(function (b) { b.setAttribute("aria-pressed", String(b === btn)); });
    ...
    function apply(cat) {
      ...
      if (!list.length) { galleryEl.innerHTML = '<p class="gallery__empty">該当する作品がまだありません。</p>'; return; }
    ...
    function cardHTML(it, i) {
      ...
      return (
        "<" + el + ' class="work' + sizeClass(it.size) + '"' + attr + ">" +
          '<div class="work__media">' +
            '<img src="' + esc(it.thumb) + '" alt="' + esc(it.title) + '" loading="lazy">' +
            '<span class="work__no">' + pad(i + 1) + "</span>" +
          "</div>" +
  ...
  function makeFilter(cat, label, pressed) {
    ...
    b.setAttribute("data-cat", cat); b.setAttribute("aria-pressed", String(!!pressed));
    b.textContent = label;
    return b;
  }
})();
